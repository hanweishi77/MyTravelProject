...
"""网页用户界面视图函数"""
# 该注释是我在测试git的功能
from flask import request, render_template, flash, redirect, url_for, session, Blueprint, jsonify
from blueprints.forms import RegisterForm, LoginForm, SuggetionForm
from models import User, UserLog, Area, Scenic, Collect, Travels, Suggestion, EmailCaptcha
...
def login():
    ...
    if form.validate_on_submit():
        ...
        session["username"] = user.username                          # 将username写入session
...
def register():
    ...
    if request.method == "GET":
        ...
        if form.validate():                                               # post表单验证成功
            data = form.data
            captcha = EmailCaptcha.query.filter_by(email=data["email"]).first()
            user = User(
            ...
            db.session.delete(captcha)                                    # 删除验证码
...
def search():
    ...
    if area_id or star:                                                   # 根据景区id,星级搜索景区
...
@home_bp.route("/mail/captcha")
def get_email_capthca():
    ...
    if user:                                                              # 如果邮箱已经存在
        ...
        # print(captcha)
        captcha = "".join(captcha)                                                                   # 四位数字随机验证码
        message = Message(subject="编程测试验证码", recipients=[email], body=f"您的验证码是：{captcha}")   # 邮箱消息
        mail.send(message)                                               # 通过邮箱服务器向用户邮箱发验证码
        email_captcha = EmailCaptcha(email=email, captcha=captcha)       # 存储邮箱和验证码
        db.session.add(email_captcha)                                    # 加入数据库会话
        db.session.commit()                                              # 提交数据库
        data = dict(ok=1)                                                # 写入字典
    import json                                                          # 引入json模块
    return json.dumps(data)                                              # 返回json格式数据
